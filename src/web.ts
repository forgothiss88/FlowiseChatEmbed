    ...
    return {
      ...
      theme: {
        ...
        chatWindow: {
          ...
          botMessage: {
            backgroundColor: props.theme?.botMessage?.backgroundColor || '#ffffff',
            textColor: props.theme?.botMessage?.textColor || '#283E4D',
            avatarSrc: props.theme?.botMessage?.avatarSrc || props.avatarSrc,
            avatarPadding: props.theme?.botMessage?.avatarPadding || '8px',
            showAvatar: props.theme?.botMessage?.showAvatar || true,
            enableMultipricing: props.theme?.botMessage?.enableMultipricing || false,
            purchaseButtonText: props.theme?.botMessage?.purchaseButtonText || 'Buy now',
            purchaseButtonBackgroundColor: props.theme?.botMessage?.purchaseButtonBackgroundColor || '#283E4D',
            purchaseButtonTextColor: props.theme?.botMessage?.purchaseButtonTextColor || '#ffffff',
            faviconUrl: props.theme?.botMessage?.faviconUrl || undefined,
          },
          userMessage: {
            backgroundColor: props.theme?.userMessage?.backgroundColor || '#202124',
            textColor: props.theme?.userMessage?.textColor || '#ffffff',
          },
          textInput: {
